//dotenv package is used to read and set environment variables
require("dotenv").config();

// Require below node package to help run the app
var request = require("request");
//import the Spotify package to send requests to the Spotify API.
var Spotify = require('node-spotify-api');
//import the Twitter package to send requests to the Twitter API.
var Twitter = require('twitter');
//import the  columnify package to display command line help output in columns.
var columnify = require('columnify')
//import the figlet package to create drawings from text.
var figlet = require('figlet');
// fs is a core Node package for reading and writing files
var fs = require("fs");

//Access keys from keys.js file
var keys = require("./keys.js");

//Read all the input parameter and store it in input array
...
//Message to confirm that the request and response is added to log file.
...
function getMovieInfo() {
	...
	// Read the movie name as one string
	...
	//If no movie name is specified on the command line, then show the movie info for the movie, Incredible.
	if (!movieName) {
	...
	request("http://www.omdbapi.com/?t=" + movieName + "&apikey=trilogy", function (error, response, body) {
		...
		//If the request is successful (i.e. if the response status code is 200 and Response is not "False" in the response body)
		if (!error && response.statusCode === 200 && response.body[13] !== 'F') {
			console.log("log that" + body);
			// console.log("log resp" + JSON.stringify(response, null, 2));
			// console.log("log resp" + response.body[13]);
			//Parse the body of the JSON object that holds the movie data and display the movie info.
			var movieInfo = JSON.parse(body);
			// console.log("log this" + movieInfo);

			// Create variable to hold Internet DB Rating.
			if (movieInfo.Ratings[0].Value)
				var InternetDBRating = movieInfo.Ratings[0].Value;
			else
			// If no rating available for the movie, then display the message
				var InternetDBRating = "No Ratings available";
			...
			var movieResult =
				...
				//Internet Movie Database rating of the movie.
				"Internet Movie Database Rating (out of 10): " + InternetDBRating + "\r\n" +
...
function getLatestTweets() {
	...
	//Parameters. Show the tweets from my timeline. Limit to the last 10 tweets.
	var params = { screen_name: 'jyotirit', limit: 10 };
	client.get('statuses/user_timeline', params, function (error, tweets, response) {
		if (!error) {
			//console.log(tweets);
			//Show last 20 tweets from my timeline.
			console.log("My last 10 tweets");
			logData("My last 10 tweets");
